...
| `POST /document/edit` (demo) | `{path, edits: [{anchor, replacement}]}` → `{before_excerpt, after_excerpt, edits_applied}` | Same anchor contract as `/documents/edit`, demo cut: `.docx`/`.pptx` run-level splice (exactly-once per anchor, 422 otherwise) + `.xlsx` whole-cell match (substring splices refused with the closest cell as a hint); temp-file + atomic replace; text formats edit directly in TS |
| `POST /document/create` (demo) | `{path, title, items[]}` → `{after_excerpt, size_bytes}` | Demo cut, no frozen counterpart: builds a `.docx` (title heading + one headed section per item), `.pptx` (title slide + one title/body slide per item), or `.xlsx` (one row per item) from plain strings; 200 items / 50k chars caps; post-save re-open check; text files go through `write_file`, PDFs through Electron `printToPDF` (docs/02 §2.6) |
...
|---|---|---|
| `POST /document/extract` | `{path}` → `{text, truncated}` | **pypdf + python-docx instead of Docling** (MVP_PLAN's call; Devlog deviation, STACK.md rows updated). `.pdf .docx .pptx .xlsx .txt .md .csv` (slides in order with `--- Slide N ---` markers + tables as tab-rows; sheets as `--- Sheet: name ---` + tab-rows; legacy `.xls`/`.ppt` get an honest 422); images are NOT extracted here — `read_document` describes them via the run's vision model TS-side (pixels never cross to the sidecar, keys never leave main). 200k-char honest cap; source guards (20 MB file cap, 500-page PDF cap, 20k-block .docx/.pptx caps, 50-sheet / 100k-row .xlsx caps, 10 MB image cap — 413 past them, extraction stops early past the char cap); 404 missing (basename only in the detail — the absolute caller path must never ride it, 2026-09-17) / 422 unsupported. Docling's structured `/documents/parse` (M4.7) supersedes this. |
| `POST /embed/embed` | `{texts: [str]}` → `{vectors: [[f32]]}` | NOT in any prior contract — added for the MVP standout feature: on-device `all-MiniLM-L6-v2` via **fastembed** (Apache-2.0, STACK.md row). Batch caps 256 texts / 400k chars; 503 when the model can't load; lazy import + lazy build per §1. S1-002 note: the model cache is the persistent `<userData>/fastembed` (seeded once per machine); a fresh/empty profile with no network stays 503 and `semantic_search` degrades honestly — environment, not a bug. |
