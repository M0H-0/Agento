...
| `POST /document/create` (demo) | `{path, title, items[]}` → `{after_excerpt, size_bytes}` | Demo cut, no frozen counterpart: builds a `.pptx` (title slide + one title/body slide per item) or `.xlsx` (one row per item) from plain strings; 200 items / 50k chars caps; text files go through `write_file` |
