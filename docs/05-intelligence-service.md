...
|---|---|---|
| `POST /document/extract` | `{path}` → `{text, truncated}` | **pypdf + python-docx instead of Docling** (MVP_PLAN's call; Devlog deviation, STACK.md rows updated). `.pdf .docx .txt .md` only; 200k-char honest cap; source guards (20 MB file cap, 500-page PDF cap, 20k-block .docx cap — 413 past them, extraction stops early past the char cap); 404 missing / 422 unsupported. Docling's structured `/documents/parse` (M4.7) supersedes this. |
