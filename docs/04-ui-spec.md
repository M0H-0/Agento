...
Step rows: status glyph (○ / spinner / ✓ / ✗ / ⚠ awaiting approval), plain description, risk accent ≥ 2; verification badge when done ("verified ✓" / "not verified"); failed → missed requirements in one sentence + retry. Header: `Start ▶` pre-execution (legacy plan-first path only), "updated" chip on plan revisions, "read-only" chip on plans from a Plan-mode run. Footer: "3 of 5 steps done" — or, for read-only plans, "Read-only plan — nothing was changed. Switch to Act and say “go ahead” to carry it out." (Arabic UI: «تابع», and the Act trigger accepts it); per-step `undo ↩`. A fresh `plan/created` is read-only; the first traced execution event (Act `plan/step_updated`, `verification/finished`, or `approval/requested` under the new runId) rebinds the panel to the executing run and flips the footer to N-of-M (2026-09-17: step updates used to require the original runId, so an Act "go ahead" — a new run — left the checklist on pending forever).
...
- WriteFileCard / CreateDirCard — per-tool card bodies on the `setToolUI` slots showing the written content only (write_file) and created/existed state (create_dir), plain-language titles from the registry `describe()` — a whole-file write shows one block ("New file" / "Replaced content — undo available in Changes"), never a before/after pair; targeted edits keep before/after in EditFileCard/EditDocumentCard — assistant-ui ships the slots but no file-tool bodies (M2.5).
...
- History (2026-09-12, user-requested standout) — **HistorySection** (Changes panel: arrow stepper walking the session's checkpoint stops oldest-first with a before-excerpt preview per stop + "Restore to here" behind the panel's inline confirm, fanning out over the existing `changes:undo` channel newest-group-first) + pure `chat/history.ts` (grouping/stop-building/restore-plan, vitest-covered) — no IPC/preload/main/storage changes (excerpts already ride `changes:list`), no new deps.
- Word-by-word streaming (2026-09-12, user-requested) — assistant prose reveals ~2 words per 40ms tick (medium pace) via a custom animator in `MarkdownText` riding the primitive's `preprocess` with `smooth={false}` (the built-in `useSmooth` catches up in ≤250ms, which still reads as chunk-at-once on large provider deltas) + pure `chat/word-pacing.ts` (word budget, whitespace-preserving tokens, ``` fence counting — slices landing mid-fence extend to the closing fence so code blocks pop atomically, unclosed fences flush whole; vitest-covered) — settled/history parts, non-prefix transitions, Stop/abort, and `prefers-reduced-motion: reduce` all render full text instantly; persistence untouched (main's accumulator stores the full reply); no IPC/preload/main/storage changes, no new deps.
- Pending-approval card fix (2026-09-12, sign-off finding) — the `GenericToolCard` fallback rendered "That step failed." for steps that were still running: a tool parked on the ApprovalDialog has no result yet, so every per-tool renderer returns null and the `?? stepFailed` fallback fired unconditionally. `renderToolCard` now attaches the failure line only when the mapped status is `incomplete`; real backend errors still surface via `errorTextFor` in every state — no new copy, no new locale keys, no IPC/preload/main changes, no new deps.
- Narrow-viewport composer (2026-09-12, sign-off finding) — below 600px the absolutely positioned model chip (`max-width: 13rem`) covered the Plan/Act tabs; a media query hides the supplementary `.mode-hint` and caps the chip at `8rem` (label already ellipsizes), direction-agnostic so the `[dir='rtl']` mirror is untouched — CSS-only, existing tokens.
- Reply chips fill the composer (2026-09-13, user-reported) — a model-authored option chip ("Proceed with your best judgment…") fired on a stray Enter and the thread recorded a reply the user never wrote; `ReplyComposer` chips now **fill the draft** and move focus to the input (Enter/Send still does the sending), so every reply passes visibly through the composer before it exists — no new components, no new deps.
- `convert_document` renders the existing WriteFileCard (same result shape as `create_document` with `beforeExcerpt: null`) — no new card (2026-09-13).
- Truncation honesty (2026-09-14, Phase-1 item 1) — `GenericToolCard` renders the wrapper's oversized-result envelope (`{ outputTruncated: true }`) as a plain-language notice (`cards.largeResult` + `cards.largeResultHint`, en/ar) with the raw-JSON result block suppressed; ranked `WebSearchCard` output stays a ranked card and its meta appends `cards.truncatedPreview` when the tool self-capped below the wrapper budget (the envelope stays a backstop). The wrapper's envelope key is `outputTruncated`, deliberately distinct from tool-level `truncated` flags (honest partial results must reach both model and card intact). Tool failures/refusals throw `ToolFailureError` so the stream's error classifier passes the tool's plain sentence through — a failed fetch never wears provider copy ("Something went wrong talking to the model provider") — no new deps.
- Listing honesty (2026-09-14, Phase-1 item 2) — a failed `list_dir` rendered "0 entries / This folder is empty" (live: `archive/` probed before it existed; assistant-ui maps a tool error to `result = { error }`, and the card's entry guard treated any record as a listing). `list_dir.execute` now returns `ok:false` with a complete plain sentence for missing ("I couldn't find the folder …") / file ("… is a file, not a folder") / unreadable targets — never the raw fs message, which embeds the absolute path; the renderer requires an actual `entries` array (`asListEntries` → null otherwise) and bails to the generic card on `isError`, so an error can never wear the empty-folder copy. `GenericToolCard` suppresses the Result block when it would repeat the error sentence — no new deps.
- Arabic plans + live checklists (2026-09-17, user-reported) — an Arabic organize request got an English plan, the Arabic "انطلق، نفّذ الخطة" parked the run (only English matched the Act trigger, so no saved plan loaded and no step traced), and the checklist stayed on pending because Act step updates carried a new runId the panel dropped as stale. `isGoAheadMessage` now accepts the Arabic affirmation family (تابع/انطلق/نفذ/… with the «تابع» the Arabic footer names); the panel rebinds step/verify/approval events to the saved plan and flips from the read-only footer to N-of-M on the first traced event; `move_path` answers success when the source is already exactly where asked (re-run safe) instead of a red already-moved card — no new deps.
...
| text-only plan answer to a file-changing request | "I couldn't make a plan for that request, so I didn't change anything. Try rephrasing it." (never a dead "Sure!" with no file; held discovery means never content above this banner — the banner stands alone) |
...
| Act mutating request, prose with zero tool calls (after one required retry) | "I didn't take any actions, so nothing changed. Try saying which file to work on and what to do with it." |
...
Voice: present tense, first person, no exclamation marks, no emoji in system copy; numbers over vibes ("42 files moved", not "All done!"). The thread-error bubble's five provider/plan copies (table rows above from 401 down through the text-only plan answer) arrive in English over the stream and the renderer maps them to `error.*` locale keys (`localizeThreadError` — S7-001), so `ar` translates while unknown sentences pass through untouched.

**Arabic UI (Settings → Appearance → Language):** the whole renderer chrome is translated via the hand-rolled `chat/locale.ts` dictionary (no i18n dep — ~380 keys, `translate`/`fill`/`plural`/`relativeTime` helpers, `LocaleProvider` + `useLocale()` binding; missing keys are a type error). English-only by design: approval title/body (main `describe()`), slash-template prose sent to the model, and the QuickActions/example-prompt utterances. Agent reply text instead follows the request's language (2026-09-17: an Arabic request gets Arabic replies, plan summaries, and fallback plan steps — docs/03 §2). `<html lang/dir>` flips (`rtl` for Arabic); flex rows mirror automatically, physical CSS (text-align, absolute pins, rail borders, list indents, chevrons) is overridden in one `[dir='rtl']` block at the end of `main.css`; user-content names/paths render inside `<bdi>`. Arabic glyphs come from system fonts (Inter has none — no bundled font, no STACK.md change); Arabic plurals use the simplified one/two/many with dual support.
...
- **Word-by-word reveal:** live assistant prose types at ~2 words per 40ms; fenced code blocks appear whole; settled messages, history, Stop/abort flush, and `prefers-reduced-motion: reduce` show full text with no animation.
